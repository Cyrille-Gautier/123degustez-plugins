...
class WPRMP_SC_Add_To_Collection {
	...
	public static function shortcode( $output, $atts, $recipe ) {
		...
		if ( 'recipe' === $button_type ) {
			...
			$output .= '<a href="' . esc_url( $recipe_collections_link ) . '" style="' . $style . '" class="wprm-recipe-not-in-collection ' . implode( ' ', $classes ) . '" data-grid-id="' . esc_attr( $grid_id ) . '">' . $icon . __( $atts['text'], 'wp-recipe-maker' ) . '</a>';
			$style .= 'display: none;';
			$output .= '<a href="' . esc_url( $recipe_collections_link ) . '" style="' . $style . '" class="wprm-recipe-in-collection ' . implode( ' ', $classes ) . '" data-grid-id="' . esc_attr( $grid_id ) . '" data-text-added="">' . $icon_added . __( $atts['text_added'], 'wp-recipe-maker' ) . '</a>';
			$output .= '<div>';
		}

...
class Elementor_Template extends Base {
	...
	public function callback( $request ) {
		...
		$is_cached_template = isset( $args['cachedTemplate'] ) ?  $args['cachedTemplate'] : true;

		$cached = apply_filters( 'jet-blocks/rest/endpoint/elementor-template/cached', $is_cached_template, $template_id );

		$transient_key = md5( sprintf( 'jet_blocks_elementor_template_data_%s', $template_id ) );

		$template_data = get_transient( $transient_key );

		if ( ! empty( $template_data ) && !$dev && 'true' === $cached ) {

...
class Jet_Elements_Headline extends Jet_Elements_Base {
	...
	protected function render() {
		...
		if ( ! empty( $settings['first_part'] ) ) {
			...
			$first_part = sprintf( '<span class="%1$s">%2$s<span class="jet-headline__label">%3$s</span></span>%4$s', $first_classes, $before_deco_html, wp_kses_post( $settings['first_part'] ), $space );
		}
		...
		if ( ! empty( $settings['second_part'] ) ) {
			...
			$second_part = sprintf( '<span class="%1$s"><span class="jet-headline__label">%2$s</span>%3$s</span>', $second_classes, wp_kses_post( $settings['second_part'] ), $after_deco_html );
		}

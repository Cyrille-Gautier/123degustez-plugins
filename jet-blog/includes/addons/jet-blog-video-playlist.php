...
class Jet_Blog_Video_Playlist extends Jet_Blog_Base {
	...
	protected function render() {
        ...
        $vd = jet_blog_video_data();

        if ( ! empty( $vd->api_error ) && current_user_can( 'manage_options' ) ) {
            if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
                echo '<div class="jet-blog-api-error" style="color:#d00;border:1px solid #d00;padding:10px;margin-top:10px;">'
                    . esc_html( $vd->api_error )
                    . '</div>';
            }
        }
	}
	...
	public function _get_video_data_atts( $video_data, $item, $settings, $index ) {
        ...
        $provider_raw = isset( $video_data['provider_name'] ) ? (string) $video_data['provider_name'] : '';
        $provider     = strtolower( (string) sanitize_text_field( $provider_raw ) );

        $video_id   = isset( $video_data['video_id'] ) ? (string) $video_data['video_id'] : '';
        $html_raw   = isset( $video_data['html'] ) && is_string( $video_data['html'] ) ? $video_data['html'] : '';
        $html_sanit = $html_raw !== '' ? wp_kses( $html_raw, $allowed_tags ) : '';
        $html_attr  = $html_sanit !== '' ? $this->adjust_height( $html_sanit ) : '';

        $data = array(
            'data-id'          => isset( $item['_id'] ) ? sanitize_key( $item['_id'] ) : '',
            'data-video_id'    => sanitize_text_field( $video_id ),
            'data-provider'    => $provider,
            'data-html'        => wp_json_encode( $html_attr ),

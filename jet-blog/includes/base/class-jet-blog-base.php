...
abstract class Jet_Blog_Base extends Widget_Base {
	...
	public function _render_meta( $position = '', $base = '', $context = array( 'before' ), $settings = array() ) {
		...
		foreach ( $meta_config as $meta ) {
			...
			$meta_callbacks = jet_blog_tools()->allowed_meta_callbacks();
			...
			$icon = $this->_get_icon( 'item_icon', $meta, '%s' );

			$meta_icon = ! empty( $icon ) && $meta_val 
				? sprintf( '<div class="%1$s__item-icon">%2$s</div>', esc_attr( $base ), $icon )
				: '';

			$meta_image = '';

			if ( ! empty( $meta['item_image']['url'] ) && $meta_val ) {
				$meta_image = sprintf( '<img class="%1$s__item-image" src="%2$s" alt="">', esc_attr( $base ), esc_url( $meta['item_image']['url'] ) );
			}

			$label = ! empty( $meta['meta_label'] )
				? sprintf( '<div class="%1$s__item-label">%2$s</div>', esc_attr( $base ), esc_html( $meta['meta_label'] ) )
				: '';

			$result .= sprintf(
				'<div class="%1$s__item">%4$s%2$s<div class="%1$s__item-value">%3$s</div></div>',
				$base, $label, $meta_val, 								
				filter_var( $meta['item_use_image'], FILTER_VALIDATE_BOOLEAN ) ? $meta_image : $meta_icon,
			);
	...
	public function _add_meta_controls( $position_slug, $position_name ) {
		...
		$repeater->add_control(
			'item_use_image',
			array(
				'label'        => esc_html__( 'Use Image?', 'jet-blog' ),
				'type'         => Controls_Manager::SWITCHER,
				'label_on'     => esc_html__( 'Yes', 'jet-blog' ),
				'label_off'    => esc_html__( 'No', 'jet-blog' ),
				'return_value' => 'yes',
		...
		$repeater->add_control(
			$this->_new_icon_prefix . 'item_icon',
			array(
				'label'            => esc_html__( 'Icon', 'jet-blog' ),
		...
		$repeater->add_control(
			'item_image',
			array(
				'label'   => esc_html__( 'Image', 'jet-blog' ),
		...
		$repeater->add_control(
			...
			array(
				...
				'type'    => Controls_Manager::SELECT,
				'default' => '',
				'options' => jet_blog_tools()->allowed_meta_callbacks(),
			)
		);
	...
	public function _add_meta_style_controls( $position_slug, $position_name, $base ) {
		...
		$this->_add_control(
			$position_slug . '_meta_icon_style',
			array(
				'label'     => esc_html__( 'Meta Icon', 'jet-blog' ),
				'type'      => Controls_Manager::HEADING,
				'separator' => 'before',
			)
		);
		...
		$this->_add_responsive_control(
			$position_slug . '_meta_margin_icon',
			array(
				'label'      => esc_html__( 'Icon Margin', 'jet-blog' ),
				'type'       => Controls_Manager::DIMENSIONS,
				'size_units' => array( 'px', '%', 'em' ),
				'selectors'  => array(
					'{{WRAPPER}} .' . $base . '__item-icon' => 'margin: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				),
			),
			75
		);
		...
		$this->_add_control(
			$position_slug . '_meta_color_icon',
			array(
				'label'     => esc_html__( 'Icon Color', 'jet-blog' ),
				'type'      => Controls_Manager::COLOR,
				'selectors' => array(
					'{{WRAPPER}} .' . $base . '__item-icon' =>'color: {{VALUE}}',
				),
			),
			25
		);
		...
		$this->_add_responsive_control(
			...
			array(
				...
				'selectors'  => array(
					'{{WRAPPER}} .' . $base . '__item-icon' => 'font-size: {{SIZE}}{{UNIT}}',
				),
			),
			50
		);
		...
		$this->_add_responsive_control(
			$position_slug . '_meta_margin_image',
			array(
				'label'      => esc_html__( 'Image Margin', 'jet-blog' ),
				'type'       => Controls_Manager::DIMENSIONS,
				'size_units' => array( 'px', '%' ),
				'selectors'  => array(
					'{{WRAPPER}} .' . $base . '__item-image' => 'margin: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				),
			),
			75
		);
		...
		$this->_add_responsive_control(
			$position_slug . '_meta_width_image',
			array(
				'label'      => esc_html__( 'Image Width', 'jet-blog' ),
				'type'       => Controls_Manager::SLIDER,
				'size_units' => array(
					'px', 'em', '%',
				),
				...
				'selectors'  => array(
					'{{WRAPPER}} .' . $base . '__item-image' =>'width: {{SIZE}}{{UNIT}}',
				),
			),
			75
		);

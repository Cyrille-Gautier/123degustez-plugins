	...
	protected function register_controls() {
		...
		$repeater->add_control(
			...
			array(
				...
				'default'     => 'plain',
				'options'     => array(
					'plain'    => esc_html__( 'Plain', 'jet-tricks' ),
			...
			<div class="jet-hotspots__inner"><?php
				...
				<div class="jet-hotspots__container"><?php
					...
					foreach ( $hotspots as $index => $hotspot ) {
						...
						$content_type    = ! empty( $hotspot['hotspot_content_type'] ) ? $hotspot[ 'hotspot_content_type' ] : 'plain';
						$tooltip_content = '';

						switch ( $content_type ) {
							case 'plain':
								$tooltip_content = $hotspot['hotspot_description'];
								break;

							case 'template':
								$template_id = $hotspot['item_template_id'];

								if ( ! empty( $template_id ) ) {
									$template_id     = apply_filters( 'jet-tricks/widgets/template_id', $template_id, $this );
									$tooltip_content = jet_tricks()->elementor()->frontend->get_builder_content_for_display( $template_id );
